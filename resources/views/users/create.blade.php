    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-lg shadow-gray-100/20 dark:shadow-none overflow-hidden">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-gradient-to-br from-emerald-500 to-green-600 rounded-xl">
                        <i class="ti ti-user-plus text-white text-lg"></i>
                    </div>
                    <div>
                        <h3 class="font-bold text-gray-800 dark:text-gray-200">Create New User</h3>
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Add a new user account to the system</p>
                    </div>
                </div>
        ...
        <form method="POST" action="{{ route('users.store') }}" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="token_type" 
                        ...
                        <option value="personal" {{ old('token_type') == 'personal' ? 'selected' : '' }}>Personal</option>
                        <option value="business" {{ old('token_type') == 'business' ? 'selected' : '' }}>Business</option>
                        <option value="enterprise" {{ old('token_type') == 'enterprise' ? 'selected' : '' }}>Enterprise</option>
                    </select>
                    @error('token_type')
                        <p class="text-xs text-red-500 mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                <button type="submit" 
                        class="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 text-white text-sm font-medium rounded-xl transition-all shadow-md hover:shadow-lg">
